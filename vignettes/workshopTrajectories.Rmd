...
On a lineage-specific basis, there are about twice as much DE genes in the mock lineage (2120) as compared to the TGFB lineage (1136). Many of the DE genes in the TGFB condition are also DE in the Mock condition, around 80%. 
...
########################
assays(sce)$logcounts <- log1p(assays(sce)$counts)
sceMock <- sce[ ,colData(sce)$treatment_id=='Mock']
sceTGFB <- sce[ ,colData(sce)$treatment_id=='TGFB']
library(Seurat)
soMock <- as.Seurat(sceMock)
soTGFB <- as.Seurat(sceTGFB)
...
sce <- SingleCellExperiment(assays=list(counts=sce$counts),
                            colData = colData(sce),
                            reducedDims = reducedDims(sce))
sce <- readRDS("../data/sce.rds")
hlp <- colData(sce)
hlp$pheno <- as.data.frame(hlp$pheno)
rownames(hlp$tradeSeq) <- rownames(hlp$pheno)
rownames(hlp$slingshot) <- rownames(hlp$pheno)

colData(sce) <- NULL
colData(sce)$pheno <- DataFrame(hlp$pheno)
colData(sce)$tradeSeq <- DataFrame(hlp$tradeSeq)
colData(sce)$slingshot <- DataFrame(hlp$slingshot)
